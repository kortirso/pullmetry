...
describe Authkeeper::Providers::Gitlab do
  ...
  before do
    allow(Authkeeper::Container.resolve('api.gitlab.auth_client')).to(
      receive(:fetch_access_token).and_return(token_response)
    )
    allow(Authkeeper::Container.resolve('api.gitlab.client')).to(
