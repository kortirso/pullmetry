...
describe Import::Synchronizers::Comments::Github, type: :service do
  ...
  context 'when there are no comments' do
    ...
    context 'when entity is ignored' do
      ...
      it 'does not create new comments' do
        expect { service_call }.not_to change(pull_request.comments, :count)
      end
    end
  ...
  context 'when there is 1 existing comment' do
    before { create :pull_request_comment, entity: entity, pull_request: pull_request, external_id: '2' }

    it 'creates 1 new comment' do
      expect { service_call }.to change(pull_request.comments, :count).by(1)
    end
  ...
  context 'when there is 1 old existing comment' do
    before { create :pull_request_comment, entity: entity, pull_request: pull_request, external_id: '1' }
    ...
    it 'creates 2 new comments and destroys old comments', :aggregate_failures do
      ...
      expect(pull_request.comments.where(external_id: 1)).to be_empty
    end
  end
end
