...
describe SlackWebhooks::Company::LongTimeReviewReportPayload, type: :service do
  ...
  context 'with pull requests' do
    ...
    it 'generates payload' do
      expect(
        service_call.dig(:blocks, 0, :text, :text).include?("*Repository long time review of #{repository.title}*")
      ).to be_truthy
    end
  end
end
